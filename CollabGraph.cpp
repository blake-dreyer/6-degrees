...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist &artist) const
{
    ...
    Vertex *vertex      = graph.at(artist.get_name());
    int counter = vertex->neighbors.size();
    for(int i = 0; i < counter; i++){
        result.push_back(vertex->neighbors[i].neighbor);
    }
                                            ...
                                            const Artist &dest) const
{
    ...
    stack<Artist> path;
    path.push(dest);
    Artist next = get_predecessor(dest);
